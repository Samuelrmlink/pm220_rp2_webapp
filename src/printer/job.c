...
static bool layout_and_build(const uint8_t *src, int src_wbytes, int src_w, int src_h,
    ...
    int out_w = src_w < avail_w ? src_w : avail_w;
    int out_h = src_h < avail_h ? src_h : avail_h;
    ...
    for (int y = 0; y < out_h; y++) {
        for (int x = 0; x < out_w; x++) {
            if (fb_get(src, src_wbytes, src_w, src_h, x - shift_x, y - shift_y)) {
    ...
    *out_len = n;
    printf("job: %u bytes BITMAP %dx%d at %d,%d\n", (unsigned)n, out_w, out_h,
           origin_x, origin_y);
    return true;
}
...
int job_media_json(char *buf, size_t cap) {
    return snprintf(buf, cap,
                    ...
                    "\"safe_margin_mm\":%g,\"width_dots\":%d,\"height_dots\":%d,"
                    "\"width_bytes\":%d,\"density\":%d}",
